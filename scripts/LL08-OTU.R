...
comm_core <- OTU_core %>% 
  ...
  .[ colSums(.)>0, ]
print("Number of OTUs (columns) in core OTU table for LL 2008")
ncol(comm_core)
...
comm_rare <- OTU_rare %>% 
  subset(., rownames(.) %in% rownames(XY_sub)) %>%
  .[ , colSums(.)>0 ]
print("Number of OTUs (columns) in rare OTU table for LL 2008")
ncol(comm_rare)
...
print("Number of OTUs (columns) in full OTU table for LL 2008")
ncol(comm_full)
