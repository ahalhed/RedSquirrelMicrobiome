...
jaccard_location_anosim <- anosim(dm_phylo_bray$data, 
              dispersal_metadata$Unique.Locations, 
              permutations = 1000)
summary(jaccard_location_anosim)
plot(jaccard_location_anosim)
```
```{r}
# full data (all samples)
jaccard_location_adonis <- adonis(dm_phylo_bray$data ~ Unique.Locations*CollectionDate, 
              data = dispersal_metadata, 
              permutations = 1000)
summary(jaccard_location_adonis)
# need to figure out how to get the plot to work (and how to look at the)
plot(jaccard_location_adonis)
```
...
df_jaccar_neighbours$Distance.X <- ifelse(
  df_jaccar_neighbours$Grid1 != df_jaccar_neighbours$Grid2,
    NA, # not calculating distance between different grids (only within grids)
  ifelse(df_jaccar_neighbours$LocationX1<0 & df_jaccar_neighbours$LocationX2>0,
  abs(df_jaccar_neighbours$LocationX2 - df_jaccar_neighbours$LocationX1),
  ifelse(
    df_jaccar_neighbours$LocationX1>0 & df_jaccar_neighbours$LocationX2<0,
  abs(df_jaccar_neighbours$LocationX1 - df_jaccar_neighbours$LocationX2), 
  abs(df_jaccar_neighbours$LocationX1 - df_jaccar_neighbours$LocationX2)
  )))
# distance between X points
df_jaccar_neighbours$Distance.Y <- ifelse(
  df_jaccar_neighbours$Grid1 != df_jaccar_neighbours$Grid2,
    NA, # not calculating distance between different grids (only within grids)
  ifelse(df_jaccar_neighbours$LocationY1<0 & df_jaccar_neighbours$LocationY2>0,
  abs(df_jaccar_neighbours$LocationY2 - df_jaccar_neighbours$LocationY1),
  ifelse(
    df_jaccar_neighbours$LocationY1>0 & df_jaccar_neighbours$LocationY2<0,
  abs(df_jaccar_neighbours$LocationY1 - df_jaccar_neighbours$LocationY2), 
  abs(df_jaccar_neighbours$LocationY1 - df_jaccar_neighbours$LocationY2)
  )))
```

```{r}
# add information about neighbours (level dependent on distance apart)
...
df_jaccar_neighbours$Neighbour <- factor(df_jaccar_neighbours$Neighbour,
    levels = c('Sample Sample', 'Immediate Neighbour', 'Near Neighbour', 'Neighbour', 'Far Neighbour', 'Same Grid', 'Different Grid'),ordered = TRUE)
# plot
...
```{r}
# jaccard
# how to take into account the neighbour thing? 
# 
vegan::anosim(df_jaccar_neighbours$Distance, 
              df_jaccar_neighbours$Neighbour, 
              permutations = 1000) %>%
  summary
```
