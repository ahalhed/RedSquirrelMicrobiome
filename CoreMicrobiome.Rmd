...
```{r}
sums<-OTU_full %>% colSums() %>% as.data.frame


sums %>% rename("Count" = ".") %>% 
  mutate(RelativeAbundance = .$Count/nrow(sums)) %>%
...
```{r}
colMeans(RA_full > 0.1) %>% as.data.frame %>% 
  rownames_to_column(var = "OTU") %>% rename("RA" = ".")
...
plyr::numcolwise(nonzero)(RA_full) %>% select_if(~max(.)>th)
```
