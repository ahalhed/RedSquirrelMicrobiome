...
this will actually take non-numeric metadata (but won't take missing data). avoids having to shift the format of the OTU data around, since it takes the phyloseq object. also doesn't randomly drop samples

```{r}
# ordinate for bray-curtis
# could also give it a distance matrix to save computing here (rs_dm_bray$data)
ordinate(ps, "CAP", "bray", ~Year+Grid+Age+Sex) %>%
  # plot
  plot_ordination(ps, ., "samples", color="Grid",
                  shape = "Sex") +
  stat_ellipse(aes(group=Grid)) +
...
ordinate(ps, "CAP", "jaccard", ~Year+Grid+Age+Sex) %>%
  # plot
  plot_ordination(ps, ., "samples", color="Grid",
                  shape = "Sex") +
  stat_ellipse(aes(group=Grid)) +
