...
```{r}
# axes 1 and 2
rs_3_2 <- ggplot(rs_PCoA_V, aes(x=PC1, y=PC2,
                                      group = Squirrel.ID, 
                                      shape = Grid)) + 
  geom_point() + # points are samples
  theme_bw() + 
  stat_ellipse(aes(group=FoodSupplement,
                   colour=FoodSupplement)) #+ stat_ellipse(aes(group=Grid,colour=Grid))
rs_3_2
```
...
```{r, message=FALSE, warning=FALSE}
# axes 1 and 2
ggplot(rs_PCoA_V, aes(x=PC1, y=PC2, col = Location)) + 
  geom_point() + # points are samples
...
rs_5 <- rs_jaccard %>% filter(Jaccard > 0) %>%
  ...
  summarise_at(vars(Jaccard), funs(mean(., na.rm=TRUE)))  %>%
  # extract year and day/month information
  mutate("Year" = word(CollectionDate.x, 1, sep = "-"),
         "Date" = word(CollectionDate.x, 2, 3, sep = "-"))
View(rs_5)
```

```{r}
# look at plot, facetted by collection date and location
ggplot(rs_5, aes(x = CollectionDate.x, y = Jaccard, group = Squirrel.ID.x)) +
...
rs_5 %>%
  ...
  ggplot(., aes(x = CollectionDate.x, y = Jaccard, group = Squirrel.ID.x)) +
  geom_line() + theme_bw() + 
