...
# this cca takes into account neighbours (weighted pcnm), time (grouped by year; month is in model),
# space (grouped by grid too), and individual factors (sex and age in the model)
Wpcnm_all <- mapply(function(x, y) PCNM_CCA(x, ps, y), XY_all, numeric_all)
```
...
```{r}
# full XY
XY <- rbind(XY_08_AG, XY_08_CH, XY_08_JO, XY_08_KL, XY_09_KL, XY_10_KL, XY_08_LL, XY_08_SU)
# remove unnescessary XY objects
rm(XY_08_AG, XY_08_CH, XY_08_JO, XY_08_KL, XY_09_KL, XY_10_KL, XY_08_LL, XY_08_SU)
```

```{r}
# combine numeric data into a single df
numeric_df <- rbind(numeric_08_AG, numeric_08_CH, numeric_08_JO, numeric_08_KL, numeric_09_KL, numeric_10_KL, numeric_08_LL, numeric_08_SU)
...
```{r}
# for full data set
e_dist <- dist(numeric_df, method = "e")
# unweighted pcnm
pcnm_full <- pcnm(e_dist, 5)
# weighted pcnm and cca
Wpcnm_full <- PCNM_CCA(XY, ps, numeric_df)
summary(Wpcnm_full)
#
```
...
Working in SHARCNET, I saved the ordisplom plots using this code (adding the figure numbers where the * is)
```{r}
...
ordisplom(pcnm_all[[1]], choices=1:4, main = "AG 2008")
```
...
# Plot first PCNMs against each other
ordisplom(pcnm_all[[2]], choices=1:4, main = "CH 2008")
```
...
# Plot first PCNMs against each other
ordisplom(pcnm_all[[3]], choices=1:4, main = "JO 2008")
```
...
# Plot first PCNMs against each other
ordisplom(pcnm_all[[4]], choices=1:4, main = "KL 2008")
```
...
# Plot first PCNMs against each other
ordisplom(pcnm_all[[5]], choices=1:4, main = "KL 2009")
```
...
# Plot first PCNMs against each other
ordisplom(pcnm_all[[6]], choices=1:4, main = "KL 2010")
```
...
# Plot first PCNMs against each other
ordisplom(pcnm_all[[7]], choices=1:4, main = "LL 2008")
```
...
# Plot first PCNMs against each other
ordisplom(pcnm_all[[8]], choices=1:4, main = "SU 2008")
```
...
#### Biplots
For the full data set (not worrying about year here)
```{r}
# CCA-full for unscaled
# need vegan loaded for plots to worked
pdf(file = "/home/ahalhed/red-squirrel-w2020/R-env/plots/CCA-scaled.pdf")
par(mfrow=c(2,1))
plot(Wpcnm_full, scaling ="species", main = "Scaled by Species", select = "Month")
plot(Wpcnm_full, scaling = "sites", main = "Scaled by Sites")
dev.off()
```
